...
const routeTree = rootRoute.addChildren([matrixRoute, designSystemRoute])
...
function renderMatrix(initial = "/?building_type_id=1") {
  const rootRoute = createRootRoute({ component: () => <Outlet /> })
  ...
  const router = createRouter({
    routeTree: rootRoute.addChildren([matrixRoute]),
...
describe("MatrixScreen", () => {
  it("рисует групповые шапки, вендора со звездой, требование и заголовок раздела", async () => {
    ...
    expect(screen.getByText("Оборудование / ОВиК")).toBeInTheDocument()
    // шапки классов:
    ...
    <div className="flex flex-col gap-4 p-6 text-foreground">
      ...
      <div className="flex items-center gap-3 text-caption text-muted-foreground">
        ...
        <Button
          ...
          onClick={() => navigate({ search: (p) => ({ ...p, offset: Math.max(0, p.offset - 50) }) })}
        >
          Назад
        </Button>
        <Button
          variant="outline"
          disabled={(matrix.data?.offset ?? 0) + 50 >= (matrix.data?.total ?? 0)}
          onClick={() => navigate({ search: (p) => ({ ...p, offset: p.offset + 50 }) })}
        >
          Вперёд
        </Button>
      </div>
...
> **Примечание для реализатора:** тест из Step 2 строит собственный router (без loader'а), поэтому `matrixRoute.useSearch()`/`useNavigate()` в экране должны работать и в тестовом дереве. TanStack Router резолвит их через ближайший matched route, а не по импортируемому `matrixRoute` — если в тестовой среде возникнет предупреждение о несовпадении route id, замени в экране `matrixRoute.useSearch()` на `useSearch({ strict: false })` и `useNavigate()` из `@tanstack/react-router` (нестрогие варианты), сохранив типовую валидацию на боевом маршруте. Это единственное место, где тестовый и боевой роутер расходятся.
...
- §Тесты (звезда как есть, группировка, `total`, пустая ячейка отсутствует) → Task 2 db-тесты; фронт-кейсы → Tasks 8–9. ✔
...
- `matrixRoute.useSearch()`/`useNavigate()` в тестовом vs боевом роутере — см. примечание в Task 9 Step 4 (fallback на нестрогие варианты).
