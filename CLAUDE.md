...
- `frontend/src/screens/` — экраны: `dashboard/` (DashboardScreen + `model.ts` чистые
  ...
  `pluralStandards`/`pluralVendors`/`avatarInitial`, helpers правила «все классы» +
  ...
  Esc cancel, `doneRef` против double-submit); блок «Где разрешён» — кастомный Radix
  trigger с leading-шевроном (`group-data-[state=open]`), sunken header, guide line,
  "все классы" rollup + per-standard summary, тихая легенда; `useVendor`/`useVendorWhereAllowed`
  на хуках Query, роут `/vendors/$vendorId`), `DesignSystemShowcase.tsx`.
