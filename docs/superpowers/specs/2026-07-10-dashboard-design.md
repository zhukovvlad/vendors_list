...
> **Граница нормализатора (совместить ожидание с поведением).** Нормализация режет
> только общий хвост `(Native)`, но **не** скобки с брендом-владельцем: «ИСТРАТЕХ
> (Grundfos)» → Grundfos — это отношение `represents_id` (данные), а не задача
> регэкспа, и в кандидаты такая пара не попадёт (она уже связана владельцем).
> Срезать все скобки нельзя: «Насос (300Вт)»/«(500Вт)» дали бы ложный дубль.
