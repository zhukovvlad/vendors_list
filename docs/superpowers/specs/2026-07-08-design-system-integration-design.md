...
**Статус:** дизайн согласован; ревью №1 учтён (accent/chart/шрифты/danger/радиусы)
...
foundation, финалим палитру при их реализации.
...
- **Контраст сплошной danger.** `#CF4157` (light) + белый ≈ 4.7:1 — ок. Но
  `#E8657A` (dark) + белый ≈ 3.2:1 — ниже 4.5 для мелкого текста кнопки. Корень:
  `--destructive` в тёмной подобран как danger-**текст на тёмном фоне** (светлый
  хью), а не как **фон под белый текст**. Решение (на ревью): для сплошной danger-
  кнопки в тёмной теме либо углубить заливку (напр. до `#CF4157`), либо взять тёмный
  текст (`--destructive-foreground` в dark). Точное значение — на ревью, неверное не
  зашиваем. Мягкие использования danger (текст/бордер/бейдж) продолжают брать
  светлый `--destructive` как есть.
- **Осознанные отступления (foundation, не пиксель-перфект):**
  ...
  2. `disabled` оставляем shadcn-механизм (`opacity-50`). **Предупреждение:** на и
     без того приглушённых тёмных токенах `opacity-50` может дать почти нечитаемый
     disabled — проверить глазами в тёмной теме; при необходимости поднять до ~0.6
     или перейти на DS-реколор (border-strong / fg-subtle).
...
3. **Категориальная палитра чартов.** DS санкционирует лишь 3 хью (violet/mint/tan).
   Для 5-рядного категориального графика нужны различимые цвета; добавлять негромкие
   не-фиолетовые хью — дизайн-решение. `--chart-5` и полный набор 5+ финалим при
   реализации чартов, с участием дизайна. До тех пор — provisional.
4. Сплошная danger-кнопка в тёмной теме: углублять заливку или брать тёмный текст
   (контраст, раздел 3).
5. UI-вес текста = 400 (Suisse Medium в хендоффе нет). Ок ли жить без medium-веса в
   интерфейсе, отдавая акцентный вес Grtsk, или доставать Suisse Medium отдельно.
