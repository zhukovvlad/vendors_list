...
**Files:**
- Modify: `frontend/src/index.css` (заменить calc-радиусы в `@theme inline`; добавить `--color-border-strong` в `@theme inline`; новый `@theme`-блок с бренд-цветами, типографикой, тенями; `--radius` в `:root`)

**Interfaces:**
- Consumes: ничего.
- Produces: утилиты `bg-violet`/`bg-violet-bright`/`bg-violet-deep`/`bg-mint`/`bg-tan`/`bg-lavender`/`text-*`; `border-border-strong`; `text-display`/`text-h1`/`text-h2`/`text-h3`/`text-body-lg`/`text-body`/`text-small`/`text-caption`; `shadow-elevation-1|2|3`/`shadow-glow-violet`/`shadow-focus-ring`; `rounded-sm|md|lg|xl` = 2/4/8/10px. Тени в `@theme` заданы светлыми (soft) значениями — тёмные переопределит `.dark` в Task 3.
...
- [ ] **Step 2: Добавить маппинг `border-strong` в `@theme inline`**

В том же `@theme inline` добавить строку рядом с `--color-border` (значение придёт из bridge в Task 3):
```css
    --color-border-strong: var(--border-strong);
```

- [ ] **Step 3: Сменить базовый `--radius`**

В `frontend/src/index.css`, блок `:root`, строка `--radius`.
...
- [ ] **Step 4: Добавить `@theme`-блок DS-экстра**

В `frontend/src/index.css`, **после** блока `@theme inline` (и до `:root`), добавить новый блок. Тени — светлые (soft) значения (дефолт `:root`); тёмные задаст `.dark` в Task 3.
...
@theme {
  ...
  /* ---- Shadows (LIGHT defaults; .dark overrides in Task 3) ---- */
  --shadow-elevation-1: 0 1px 2px rgba(23, 19, 31, 0.06);
  --shadow-elevation-2: 0 6px 18px rgba(23, 19, 31, 0.10);
  --shadow-elevation-3: 0 18px 50px rgba(23, 19, 31, 0.14);
  --shadow-glow-violet: 0 0 0 1px rgba(117, 74, 232, 0.30), 0 8px 30px rgba(117, 74, 232, 0.20);
  --shadow-focus-ring:  0 0 0 3px rgba(117, 74, 232, 0.18);
}
```
...
> Примечание: `border-border-strong` до Task 3 резолвится в пустой `var(--border-strong)` — это не ошибка сборки; значение появится с bridge. Пока никакой компонент его не использует.
...
**Interfaces:**
- Consumes: маппинги `@theme inline` (`--color-*: var(--*)`), `--color-border-strong` (Task 2).
- Produces: переменные shadcn в hex DS для обеих тем; новые bridge-vars `--border-strong`, `--primary-hover`, `--destructive-solid`, `--destructive-foreground` для Task 4.

- [ ] **Step 1: Переписать `:root` (светлая тема)**

В `frontend/src/index.css` заменить содержимое блока `:root` (кроме уже изменённого `--radius: 0.5rem` из Task 2 — его сохранить). Полный блок:
...
В `frontend/src/index.css` заменить содержимое блока `.dark`. Включает тёмные тени (переопределяют светлые из `@theme` Task 2):
...
.dark {
    ...
    /* Shadows — dark (strong) переопределяют @theme-дефолт */
    --shadow-elevation-1: 0 1px 2px rgba(0, 0, 0, 0.4);
    --shadow-elevation-2: 0 6px 18px rgba(0, 0, 0, 0.5);
    --shadow-elevation-3: 0 18px 50px rgba(0, 0, 0, 0.6);
    --shadow-glow-violet: 0 0 0 1px rgba(117, 74, 232, 0.5), 0 8px 30px rgba(117, 74, 232, 0.35);
    --shadow-focus-ring:  0 0 0 3px rgba(117, 74, 232, 0.2);
}
```
...
describe("Button", () => {
  ...
  it("subtle-вариант: фиолетовая подложка через bg-primary/10", () => {
  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-md border border-transparent bg-clip-padding text-sm font-normal whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 dark:disabled:opacity-60 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        subtle:
          "bg-primary/10 text-[var(--primary-hover)] hover:bg-primary/20",
        destructive:
...
**3. Type consistency:** имена bridge-vars (`--primary-hover`, `--destructive-solid`, `--border-strong`) совпадают между Task 3 (определение) и Task 4 (использование). `--color-border-strong: var(--border-strong)` (Task 2) ↔ `--border-strong` (Task 3). Вариант `subtle` в cva (Task 4) ↔ ассерт `data-variant="subtle"` (тест Task 4). `text-caption`/`font-display`/`bg-violet-bright`/`bg-mint`/`bg-tan` (Task 5) определены в Task 2.
