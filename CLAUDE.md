...
- `backend/migrations/` — Alembic; `sql/` — два канонических SQL-файла (неизменны);
  новые ревизии — `versions/` чистым SQL (первая — `0003_category_sort_path`).
...
- `frontend/src/router.tsx` — code-based дерево TanStack Router (`/` — матрица,
  `/design-system` — витрина DS); `routeTree` экспортируется для memory-router в тестах.
- `frontend/src/screens/` — экраны: `matrix/` (MatrixScreen + `model.ts` чистые хелперы +
  `columns.tsx`), `DesignSystemShowcase.tsx`.
- `frontend/src/components/ui/` — DS-компоненты на токенах (`button`, `table`, `badge`,
  `card`); не-компонентные экспорты (cva-варианты) — в отдельных `*-variants.ts`
  (react-refresh lint запрещает смешивать).
...
> на матрице; следующий фронт-срез — светофор соответствия §4 (карточки проектов).
> Отложенный полиш — [docs/TECH_DEBT.md](docs/TECH_DEBT.md).
