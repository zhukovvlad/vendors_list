...
- `setup_logging() -> None` — идемпотентно (модульный флаг `_configured`).
  ...
  `LOG_DIR` (дефолт — `_default_log_dir()`).
- `_default_log_dir() -> Path` — чистый хелпер: `parents[1]/"logs"` =
  `backend/logs`. **`parents[1]`, не `parents[2]`** — файл лежит в `app/`, а не
  `app/core/`. Вынесен из `setup_logging`, чтобы юнит-тестить путь **без** вызова
  `setup_logging` и без файловой системы (иначе тест дефолта создал бы реальные
  `backend/logs/app.log` при каждом прогоне).
...
**Сессионная изоляция файлов** (обязательная): `setup_logging()` теперь первая
строка `create_app()` — значит **любой** api-тест с TestClient поднимет
приложение и при дефолте `LOG_TO_FILE=1` создаст `backend/logs` в рабочем
дереве. Фикстура логгинг-тестов это не покрывает. Выставить `LOG_TO_FILE=0`
на всю тестовую сессию: autouse session-фикстура в `conftest.py` через
`os.environ` (session-scope несовместим с `monkeypatch`), либо env в just-рецепте
`test`/CI-джобе — выбор имплементации.
...
- **`test_logging_config.py`** (ориентир — CIW `test_logging_config.py`):
  ...
  без бинда, значение после `bind_request_id`); `uvicorn.access` = WARNING и
  `propagate=True`.
- **`test_default_log_dir` (или в том же файле):** `_default_log_dir()` даёт
  путь, оканчивающийся на `backend/logs` — **без** вызова `setup_logging` и без
  создания файлов.
- **`test_middleware.py`** (api): `X-Request-ID` в ответе; переиспользование
  ...
  отсутствии; строка лога запроса присутствует (через `caplog` — `app.request`
  пропагирует до root, своих хендлеров нет; хендлер в тесте городить не нужно).
