...
- `backend/app/` — приложение: `main.py`, `config.py`, `db.py` (движок + tx),
  ...
  `vendors.py` с `PATCH /vendors/{id}` для правки имени/note/`kind`: rename→DELETE own alias →
  UPDATE name → INSERT old name ON CONFLICT DO NOTHING, идемпотентно A→B→A с финстейтом
  тестом aliases {B}; listing-мутации `POST /vendors/{id}/listings(/exclude|/restore)` — правки
  разрешений идут прямо в живой `listing`, порядок блокировок listing→release ЕДИН во всех трёх,
  `ensure_open_release` вызывается ПОСЛЕ записи и только при rowcount>0 [no-op не плодит
  фантомный черновик]; exclude — ОДИН scoped-эндпоинт {scope: class|position|standard};
  `building_type` достижим только через `segment.building_type_id`; `listing_cell_chk`→409
  через SQLSTATE **P0001** [`_is_cell_chk`, это НЕ `IntegrityError`]; `_add_one_class`
  un-delete-first-else-INSERT общий для add/restore), `meta.py` с `GET /meta/positions`
  (комбобокс «+ стандарт»), `schemas/` (Pydantic, включая `VendorHeaderUpdate` +`kind`,
  `ListingAdd`/`ListingExclude`/`ListingRestore`/`ListingExcludeResult`/`MetaPosition`),
...
- `backend/migrations/` — Alembic; `sql/` — два канонических SQL-файла (неизменны);
  ...
  `GET /vendors/{id}/where-allowed` отдаёт `segment_count` per standard для правила
  «все классы»; `0006_ensure_open_release` — функция `ensure_open_release(bt)` обеспечивает
  ЕДИНСТВЕННЫЙ open-маркер релиза на тип объекта [гонку снимает ограничение
  `uq_release_one_open`, не сервисный лок; label нейтрален — `freeze_release` его не трогает]).
...
- `frontend/src/screens/` — экраны: `dashboard/` (DashboardScreen + `model.ts` чистые
  ...
  `pluralStandards`/`pluralVendors`/`pluralClasses`/`avatarInitial`, helpers правила «все классы» +
  `isAllClasses`/`standardAllClasses` (`excluded===0 && allowed===segment_count`),
  `splitQualifier` (голова/уточнение в скобках), `excludeScaleForPosition`/`excludeScaleForStandard`
  (клиентский предрасчёт масштаба исключения);
  `InlineEditText` компонент инлайн-правки (Notion/Linear pattern: Enter/blur commit,
  Esc cancel, `doneRef` против double-submit, проп `readOnly` для view-режима); блок «Где разрешён» —
  кастомный Radix trigger с leading-шевроном (`group-data-[state=open]`), sunken header, guide line,
  "все классы" rollup + per-standard summary, типографика v3 (mint-иконка `CheckCheck`), тихая легенда;
  **режим правки view↔edit** — локальный `editMode`/`expanded`, все affordance гейтятся им (view —
  read-only дефолт), баннер режима; операции разрешений (класс-`×`/«+ класс»/позиция-`⊖`/kebab
  стандарта/«вернуть»/kind-дропдаун); `ExcludeDialog` (масштаб позиция/стандарт: preview=клиент,
  факт=сервер) и `AddStandardDialog` (стандарт→позиция→классы); мутации гасят 4 ключа
  (`vendor-where-allowed`/`vendor`/`matrix`/`dashboard`), алиасы — только `vendor`;
  `useVendor`/`useVendorWhereAllowed`/`useAddListings`/`useExcludeListings`/`useRestoreListing`/
  `useMetaPositions` на хуках Query, роут `/vendors/$vendorId`), `DesignSystemShowcase.tsx`.
- `frontend/src/components/ui/` — DS-компоненты на токенах (`button`, `table`, `badge`,
  ...
  `dropdown-menu`/`avatar`/`breadcrumb`/`sonner`/`sheet`/`tooltip`/`separator`; +примитивы
  правки `dialog`/`popover`/`command`/`checkbox`/`radio-group` [+транзитивные `input-group`/`textarea`
  от `command`] — все в УНИФИЦИРОВАННОЙ конвенции `import { X as XPrimitive } from "radix-ui"`,
  не раздельный `@radix-ui/react-*`);
...
- **Backend:** pytest + pytest-asyncio. Маркер `db` = интеграционный тест против
  ...
  `engine` + `_shared_conn` — **session-scoped** (одно соединение к Neon на всю сессию,
  единый event loop через `asyncio_default_*_loop_scope = "session"`); `db_conn` —
  своя транзакция на тест на этом общем соединении. Хендшейк к удалённому Neon платится
  один раз, не на каждый тест — локальный прогон в разы быстрее (против удалённой БД
  доминировал RTT хендшейка; CI-раннер рядом с Neon — там эффект мал). Изоляция та же.
...
3. ✅ Матрица перечня (первый продуктовый экран, PR #12): `GET /listings/matrix`
   ...
   - ✅ **Карточка вендора** (read-эндпоинты + мутации соглашения/alias, кликабельность матрицы):
     ...
     - ✅ **Полиш блока «Где разрешён»** (иерархия + правило «все классы»): backend добавляет
       `segment_count` в ответ эндпоинта; фронт реализует правило (helpers `isAllClasses`/
       `standardAllClasses`); блок переработан — кастомный Radix-триггер с leading-шевроном,
       sunken header-полоса full-bleed, guide line, "все классы" rollup в заголовок + per-standard
       summary, тихая легенда с мини-примером; снимается визуальный симптом C2 — см.
       [devlog](docs/devlog/2026-07-12-where-allowed-polish.md).
     - ✅ **Режим правки карточки + типографика v3** (view↔edit, редактирование разрешений):
       ось «черновик — не песочница» — правки идут ПРЯМО в живой `listing` (у него нет `release_id`),
       open-`release` — лишь маркер «есть неопубликованные изменения», «Готово» ничего не коммитит.
       Ревизия `0006_ensure_open_release` (единственный open-маркер на тип, гонка на
       `uq_release_one_open`); listing-мутации add/exclude/restore (порядок listing→release,
       `ensure_open_release` после записи при rowcount>0; exclude — один scoped-эндпоинт;
       `listing_cell_chk`→409 через SQLSTATE P0001) + `PATCH kind` + `GET /meta/positions`;
       фронт — `editMode` гейтит все affordance (view read-only), операции класс/позиция/стандарт
       (`ExcludeDialog` двухфазный масштаб) + «+ стандарт» (`AddStandardDialog`) + kind-дропдаун,
       5 ui-примитивов в unified-radix, типографика v3. Где живёт логика (§6-тест): единственность
       open-маркера/soft-delete/«ячейка = вендоры ЛИБО мета» — в БД (инвариант); режим/диалоги/
       предрасчёт масштаба — на фронте (экран/процесс) — см.
       [devlog](docs/devlog/2026-07-13-vendor-card-edit-mode.md).
