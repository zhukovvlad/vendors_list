...
- [ ] **Step 3: Обновить CLAUDE.md — §5, карта, раздел про docstrings**

В `CLAUDE.md`:

(3a) В разделе «Порядок работ» отметить дашборд как сделанный срез; в «Карту репозитория» добавить `screens/dashboard/`, `app/services/dashboard.py`, вьюхи `dashboard_*`, роут `/` (дашборд) / `/matrix` (матрица).

(3b) Добавить новый раздел **после раздела «Логирование»** (оба — кросс-срезовые конвенции кода) дословно:
