...
r4 — стык `q`×`segment_id` и подпись `total` при сужении; r5 — детерминизм `csp` при
дублях `sort_order` (пары `[sort_order, id]`), тест дубля заголовка, триггер материализации.
«Утверждён» относится к текущему телу (r5).
...
собирает предков в `int[]`; сравнение массивов в Postgres поэлементно-лексикографично
даёт preorder. **Детерминизм при дублях `sort_order`:** на каждый уровень предка
кладём **пару** `[sort_order, id]` (массив плоский: `[so₁, id₁, so₂, id₂, …]` от корня
к листу). Так при совпадении `sort_order` двух узлов под общим родителем tie-break
идёт по `category.id` — порядок **разделов** детерминирован, не только позиций (иначе
`p.id` в хвосте `ORDER BY` спасает лишь позиции внутри раздела, но не сами разделы).
Не полагаемся на гигиену данных `(parent_id, sort_order)`. **Новой Alembic-ревизией
чистым SQL** (`just makemigration name="category_sort_path"` + `op.execute`); базовый
`0001` неизменен (CLAUDE.md §5). Это порядок отображения — презентация, инвариант не
трогаем, звезду/светофор не пересчитываем. Первый бэкенд-слайс.
...
- db-тест `category_sort_path`: preorder-массив, кураторский порядок ≠ алфавитному;
  **детерминизм при дублях**: два раздела под общим родителем с одинаковым `sort_order`
  → устойчивый порядок по `id` (пары `[sort_order, id]` в `csp`).
...
- **Дубль заголовка на границе (явный кейс, чтоб не «починили»):** раздел, продолжающийся
  со стр. N на стр. N+1, печатает заголовок на первой строке стр. N+1 — тест утверждает
  это как ожидаемое, иначе следующий разработчик снесёт дубль как баг.
...
- **Рекурсивные `category_path`/`category_sort_path`.** v1 сводит вызовы к
  O(различные категории **на странице**) через CTE `cats`. **Триггер актуализации
  раньше «больших деревьев»:** если на реальных данных категории окажутся почти
  уникальны per-position (50 позиций ≈ 50 категорий) — экономия CTE исчезает, и
  материализация пути (доп. колонка на `category` с триггерным пересчётом /
  materialized view) понадобится раньше. Пока (позиции группируются в разделы) — нет.
