...
- Modify: `backend/pyproject.toml` (ruff `per-file-ignores` для conftest — E402)
...
- [ ] **Step 2: ruff — узаконить pre-import настройку в conftest (E402)**

`os.environ.setdefault(...)` перед импортами делает последующие импорты
«не в начале файла» → ruff E402 (входит в `select=["E"]`, `per-file-ignores`
сейчас нет — линт упал бы на `just ci`). Весь смысл conftest здесь — pre-import
настройка, это законное исключение. В `backend/pyproject.toml`, в секцию
`[tool.ruff.lint]`, добавить после `ignore = [...]`:

```toml
[tool.ruff.lint.per-file-ignores]
"tests/conftest.py" = ["E402"]  # os.environ.setdefault до импорта app.main — намеренно
```

Проверка линта прямо здесь (не откладывая на Task 4):

Run: `cd backend; uv run ruff check tests/conftest.py`
Expected: `All checks passed!`

- [ ] **Step 3: main.py — setup_logging + middleware, убрать basicConfig**
...
- [ ] **Step 4: config.py — удалить осиротевшее поле**
...
- [ ] **Step 5: seed_vendors.py — логи в CLI**
...
- [ ] **Step 6: Прогнать весь backend — ничего не сломалось, файлы не появились**
...
```bash
git add backend/tests/conftest.py backend/pyproject.toml backend/app/main.py backend/app/config.py backend/scripts/seed_vendors.py
