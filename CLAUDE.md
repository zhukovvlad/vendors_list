...
- `backend/app/` — приложение: `main.py`, `config.py`, `db.py` (движок + tx),
  ...
  `dashboard.py`: детект похожих вендоров по норм-коллизии, timeout→`None`),
...
- `frontend/src/screens/` — экраны: `dashboard/` (DashboardScreen + `model.ts` чистые
  ...
  `columns.tsx`), `vendors/` (VendorsScreen-заглушка), `DesignSystemShowcase.tsx`.
- `frontend/src/components/ui/` — DS-компоненты на токенах (`button`, `table`, `badge`,
  `card`, `skeleton`, + shadcn-примитивы оболочки `sidebar`/`dropdown-menu`/`avatar`/
  `breadcrumb`/`sonner`/`sheet`/`tooltip`/`separator`); не-компонентные экспорты
  (cva-варианты) — в отдельных `*-variants.ts` (react-refresh lint запрещает смешивать;
  где хук/варианты соседствуют с компонентами — `eslint-disable
