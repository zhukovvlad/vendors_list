...
спека [.../specs/2026-07-12-vendor-card-polish-inline-edit-design.md](../superpowers/specs/2026-07-12-vendor-card-polish-inline-edit-design.md)):
...
- **`useUpdateVendorHeader` хук** — мутирует PATCH, инвалидирует three queryKeys:
  ...
  - `["dashboard"]` — обновляет дашборд (детект дублей вендоров по норм-имени мог измениться)

- **Helper functions в `model.ts`:**
  - `pluralStandards(count)` — склонение "1 стандарт", "2–4 стандарта", "5+ стандартов"
  - `pluralVendors(count)` — склонение "1 вендор", "2–4 вендора", "5+ вендоров"
  - `avatarInitial(name)` — выделение инициала для аватара (первая буква имени)
  - Fix RU-copy: теперь "N вендоров представляют этот бренд" со склонением (was "N брендов представлены этим")

### UI/DS

- **DS-компоненты** (без новых): `switch` (toggle), `accordion` (развёртки), `badge`/`button`.
  Инлайн-правка (`InlineEditText`) и поле алиаса — на нативных `<input>`/`<textarea>` с токен-классами
  Tailwind (отдельный shadcn-примитив `input` не заводили).
...
**Решение:** final-state тест `test_patch_name_round_trip_alias_state` в `test_vendors.py`
проверяет, что после A→B→A в БД остаётся только alias {B}, нет двойной записи A.
...
карточки) требует `text-h3` (выше по размеру). Правка применена отдельным коммитом реализации
(`fix(vendors): аватар-инициал — text-h3 вместо несуществующего text-h4`).
...
- `just ci` зелёный: backend 158 passed, frontend 81 passed (19 файлов),
  ...
  - api-тесты: `test_vendors.py::test_patch_name_round_trip_alias_state` (финстейт A→B→A)
