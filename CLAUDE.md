...
- `frontend/src/router.tsx` — code-based дерево TanStack Router (`/` — дашборд «Обзор»,
  `/matrix` — матрица, `/design-system` — витрина DS); `routeTree` экспортируется для
  memory-router в тестах.
...
- `frontend/src/components/ui/` — DS-компоненты на токенах (`button`, `table`, `badge`,
  `card`, `skeleton`); не-компонентные экспорты (cva-варианты) — в отдельных `*-variants.ts`
