...
- **`isAllClasses(position, segmentCount): boolean`** — для einzeler позиции:
  ```typescript
...
- **Тонкий кейс:** покрытие + несколько исключений = "все классы"? Нет —
  правило требует `excluded===0` (zero exceptions), отложенный кейс в TECH_DEBT.
...
- Иначе (исключённых нет) → просто `whereAllowedLegend(false)`, т.е. текст
